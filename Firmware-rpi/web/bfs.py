import socket
import time
import json
import os
import sys

LAB_SIZE = 33  

class MonitorState:
    def __init__(self):
        self.lab = [[0 for _ in range(LAB_SIZE)] for _ in range(LAB_SIZE)]
        self.robot_x, self.robot_y = 8, 8
        self.lab[self.robot_y][self.robot_x] = 3  
        self.corn = 0
        self.input_logs = ["System thin walls 33x33 started."]
        self.found_victims = []

mon_state = MonitorState()
sock_out = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
MONITOR_IP = "127.0.0.1"  
MONITOR_PORT = 5002
bfs_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

def send_msg(text):
    bfs_sock.sendto(text.encode('utf-8'), (MONITOR_IP, MONITOR_PORT))

def send_msg_to_monitor(text: str):
    bfs_sock.sendto(text.encode('utf-8'), (MONITOR_IP, MONITOR_PORT))

def load_data_from_monitor():
    filename = "maze_shared.json"
    if os.path.exists(filename) and os.path.getsize(filename) > 0:
        with open(filename, "r", encoding="utf-8") as f:
            data = json.load(f)
            if isinstance(data, dict):
                rx = data.get("robot_x", 8)
                ry = data.get("robot_y", 8)
                lab = data.get("matrix", [[0 for _ in range(LAB_SIZE)] for _ in range(LAB_SIZE)])
                corn = data.get("corn", 0) 
                return lab, rx, ry, corn
    return [[0 for _ in range(LAB_SIZE)] for _ in range(LAB_SIZE)], 8, 8, 0

def bfs(sent):
    global curr, path, queue, robot_x, robot_y, found
    lab, robot_x, robot_y, corn = load_data_from_monitor()
    corn = int(corn)

    if 0 <= robot_y < LAB_SIZE and 0 <= robot_x < LAB_SIZE:
        lab[robot_y][robot_x] = 1
    else:
        return

    send_msg_to_monitor("start bfs")
    found = False
    neimber = [[0, -2], [2, 0], [0, 2], [-2, 0]]
    neimber_wall = [[0, -1], [1, 0], [0, 1], [-1, 0]]

    queue = [[robot_x, robot_y]]
    path = [-1]
    curr = 0

    while curr < len(queue):
        nx, ny = queue[curr][0], queue[curr][1]
        if lab[ny][nx] == sent and (nx != robot_x or ny != robot_y):
            found = True
            break
            
        for ab in range(len(neimber)):
            tx = nx + neimber[ab][0]
            ty = ny + neimber[ab][1]
            wx = nx + neimber_wall[ab][0]
            wy = ny + neimber_wall[ab][1]
            
            if 0 <= ty < LAB_SIZE and 0 <= tx < LAB_SIZE and 0 <= wy < LAB_SIZE and 0 <= wx < LAB_SIZE:
                if lab[wy][wx] == 0 and [tx, ty] not in queue:
                    queue.append([tx, ty])
                    path.append(curr)
        curr += 1

def ride():
    global curr, path, queue, robot_x, robot_y
    ROBOT_PORT = 5003
    target_idx = curr
    actual_path = []
    
    while target_idx != -1:
        actual_path.append(queue[target_idx])
        target_idx = path[target_idx]
    actual_path.reverse()
    
    curr_x, curr_y = robot_x, robot_y

    for i in range(1, len(actual_path)):
        tx, ty = actual_path[i]
        dx, dy = tx - curr_x, ty - curr_y

        if dy > 0:
            cmd_char, corn = 's', 180
        elif dy < 0:
            cmd_char, corn = 'w', 0
        elif dx > 0:
            cmd_char, corn = 'd', 90
        elif dx < 0:
            cmd_char, corn = 'a', 270
        
        send_msg(f"pos:{tx // 2},{ty // 2}:{corn}")
        sock_out.sendto(f"cmd:{cmd_char}".encode('utf-8'), ('127.0.0.1', ROBOT_PORT))
        curr_x, curr_y = tx, ty
        time.sleep(1.5)

        if i < len(actual_path) - 1:
            next_tx, next_ty = actual_path[i+1]
            wall_x = (curr_x + next_tx) // 2
            wall_y = (curr_y + next_ty) // 2
            fresh_lab, _, _, _ = load_data_from_monitor()
            if fresh_lab[wall_y][wall_x] == 2 or fresh_lab[wall_y][wall_x] == 4:
                break 
            
if __name__ == "__main__":
    time.sleep(1.5) 
    bfs(0)
    if found:
        ride()
    if not found:
        bfs(3)
        ride()
        send_msg_to_monitor("finish")