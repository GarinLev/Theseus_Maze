...
def run_bfs_route(corn, rx, ry, black):
    global rx_black, ry_black
    set_camera_pause(True)
    result = subprocess.run(
        [sys.executable, bfs_path, str(rx), str(ry), str(corn), str(rx_black), str(ry_black)], 
        capture_output=True, 
        text=True
    )
    if result.stderr:
...
def main():
    global stat_bfs, stat_black, stat_cam, rx_black, ry_black, stat_blue, stat_red
    ser.write(b"s")
    rx, ry = 15, 15
    corn = 0
    chk_rx, chk_ry, chk_corn = 15, 15, 0
    prev_rx, prev_ry, prev_corn = 15, 15, 0
    ...
    while True:
        ...
        if cmd == 'p':
            ...
            while pause != 0:
                ...
                print(rcmd)
                if rcmd.startswith('o'):
                    rx, ry, corn = chk_rx, chk_ry, chk_corn
                    send_msg(f"pos:{rx},{ry}:{corn}")
                    set_camera_pause(False)
                    print("oy_its_not_pause")
                    pause = 0
                time.sleep(0.01)

        elif cmd.startswith('s'):
            ser.write(b"s")
            print("what_arduino_doing")
            time.sleep(0.05)
            continue
        ...
        if len(cmd) >= 4 and cmd[:4].isdigit():
            ...
            if stat_black == 1 and stat_red == 1 and stat_blue == 2:
                corn, rx, ry = run_bfs_route(corn, rx, ry, "black")
                send_to_arduino('e')
                send_msg("finish")

            elif n_walls >= 3 or stat_bfs == 1:
                stat_bfs = 0
                ...
                if 0 <= next_y * 2 < 60 and 0 <= next_x * 2 < 60:
