    ...
    if os.path.exists(filename) and os.path.getsize(filename) > 0:
        ...
        if content.startswith("{") and content.endswith("}"):
            ...
            if isinstance(data, dict):
                rx = data.get("robot_x", 30)
                ry = data.get("robot_y", 30)
                lab = data.get("matrix", [[0 for _ in range(LAB_SIZE)] for _ in range(LAB_SIZE)])
                corn = data.get("corn", 0)
                return lab, rx, ry, corn
    return [[0 for _ in range(LAB_SIZE)] for _ in range(LAB_SIZE)], 30, 30, 0

def bfs(sent):
    global curr, path, queue, robot_x, robot_y, found, rx_black, ry_black
    lab, r_x, r_y, corn_val = load_data_from_monitor()
    
    if len(sys.argv) >= 4:
        robot_x = int(sys.argv[1]) * 2
        robot_y = int(sys.argv[2]) * 2
        corn = int(sys.argv[3])
        rx_black = int(sys.argv[4]) * 2
        ry_black = int(sys.argv[5]) * 2
        print(f"i_put_rx_ry: X={robot_x}, Y={robot_y}", file=sys.stderr)
    else:
        print(f"were_efwj", file=sys.stderr)
    ...
    while curr < len(queue):
        ...
        if sent == 3:
            if nx == rx_black and ny == ry_black:
                found = True
                break
        else:
