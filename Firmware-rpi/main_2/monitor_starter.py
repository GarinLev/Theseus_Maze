...
state = {
    ...
    "robot_y": 30,
    "corn": 0,
    "logs": ["Web Monitor 60x60 started."],
    "victims": []
}
state["matrix"][30][30] = 3
    ...
    while True:
            ...
            for w in walls:
                ...
                if 0 <= wx < LAB_SIZE and 0 <= wy < LAB_SIZE:
                    if state["matrix"][wy][wx] == 3:
                        continue 
                    if any(v.get("mx") == wx and v.get("my") == wy for v in state["victims"]):
                        continue
        ...
        try:
            with open("maze_shared.json", "w", encoding="utf-8") as f:
                json.dump(state, f)
                #print(state["matrix"][state["robot_y"]][state["robot_x"]])
    ...
    <body>
        ...
        <script>
            ...
            function draw(state) {
                ...
                for (let y = 0; y < 60; y++) {
                    ...
                    for (let x = 0; x < 60; x++) {
                        ...
                        if (x % 2 === 0 && y % 2 === 0) {
                            ...
                            else if (val === 4) ctx.fillStyle = '#50fa7b'; 
                ...
                // Рисуем жертв поверх матрицы
                (state.victims || []).forEach(v => {
                    if (v.mx == null) return;
                    let xData = getCoords(v.mx);
                    let yData = getCoords(v.my);
                    ctx.fillStyle = '#50fa7b';
                    ctx.fillRect(xData.pos, yData.pos, xData.size, yData.size);
                });
