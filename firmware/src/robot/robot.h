#ifndef ROBOT_H
#define ROBOT_H

#include <Arduino.h>
#include "../../lib/pt/pt.h"

// --- 1. МАКРОСЫ (Должны быть в самом верху) ---
#ifndef WC_MM_TO_TICKS
#define WC_Radius 35.0f // 70 / 2
#define WC_TICKS_PER_REV 500.0f
#define WC_MM_TO_TICKS(mm) ((mm) * WC_TICKS_PER_REV / (TWO_PI * WC_Radius))
#endif
#define COM_ENABLE
// --- 2. БАЗОВЫЕ ТИПЫ ---
namespace robot {
    enum TaskRobot {
        TaskRobot_END, TaskRobot_WAIT, TaskRobot_STEP_UP, TaskRobot_STEP_LEFT,
        TaskRobot_STEP_RIGHT, TaskRobot_STEP_DOWN, TaskRobot_VICTIM_LEFT,
        TaskRobot_VICTIM_RIGHT, TaskRobot_VICTIM_LEFT_X2, TaskRobot_VICTIM_LIGHT,
        TaskRobot_VICTIM_RIGHT_X2
    };

    enum StateRobot {
        StateRobot_WAIT, StateRobot_MOVE, StateRobot_ROTATE, StateRobot_VICTIM
    };

    // Прототипы функций, чтобы robot.cpp их видел
    void update_step();
    void update_victim();
    void handleWait();
    void handleMove();
    void handleRotate();
    void handleVictim();

    // Пространство имен для энкодеров
    namespace encoder {
        void isr_A1(); void isr_A2();
        void isr_B1(); void isr_B2();
    }
}

// --- 3. ПОДКЛЮЧЕНИЕ КОНТРОЛЛЕРОВ ---
#include "../controller/controller_wheel.h"
#include "../controller/controller_wall.h"
#include "../controller/controller_rotate.h"
#include "../controller/controller_servo.h"
#include "../controller/controller_debug.h"
#include "../controller/controller_color.h"
#include "../../lib/microLED/microLED.h"

// --- 4. ОБЪЯВЛЕНИЕ ОБЪЕКТОВ И ДАННЫХ ---
namespace robot {
    extern TaskRobot task;
    extern StateRobot state;
    extern uint16_t count_steps;

    extern ColorController colorController;
    extern RotateController rotateController;
    extern ServoController servoController;
    extern WallController wallRight, wallLeft;
    extern WheelController wheelA1, wheelA2, wheelB1, wheelB2;

    extern const float OFFSETS_COLOR[8] PROGMEM; // Теперь контроллеры цвета это увидят

    void setGlobalState(StateRobot s);
}

// --- 5. МЕНЕДЖЕРЫ (Используют всё вышеперечисленное) ---
#include "../controller/manager_wall.h"
#include "../controller/manager_wheel.h"

namespace robot {
    extern WallManager wallManager;
    extern WheelManager wheelManager;

    void init();
    void loop();

    namespace encoder {
        void isr_A1(); void isr_A2();
        void isr_B1(); void isr_B2();
    }
}

#endif