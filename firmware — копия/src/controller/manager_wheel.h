...
struct WheelManager {
    ...
    struct pt pt_task;
    enum States { IDLE, MOVE_SOFT, SLOPE_UP, SLOPE_DOWN, HIT, BLACK };
    States state = IDLE;
...
/*
            if (state == SLOPE_UP || state == SLOPE_DOWN) {
                hasDouble = true;
                setAllSpeed(state == SLOPE_UP ? 70 : 60);

                start_encoder = fr->encoderTopic.value;
                PT_WAIT_UNTIL(&pt_task, (state == SLOPE_UP ? *pitch_ptr < 15.0f : *pitch_ptr > -15.0f) || state == IDLE);
                Serial.print("Encoder: "); Serial.println(fr->encoderTopic.value - start_encoder);
